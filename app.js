    ...
    input: {
     ...
     .then(res => {
      if(res.output.generic[0].options){
        reply = [];
        reply.push("Please select the fields that you are interested in:");
      ...
      if(skills){
        if(skills.categories){       
          if(skills=='science'){
            reply = [];
            reply.push("Please select which field in science that you are interested in:");
            //console.log(res.output.generic[1].options)
            res.output.generic[1].options.forEach(option => {
            reply.push(option.label);
            });
          ...
          console.log(skills.categories);
	        console.log(skills.categories[0].value);
          switch(skills.categories){
            default: queryString = skills.categories[0].value,
            ...
            if(err){
	          ...
	          console.log(queryResults);
            	...
            	console.log(queryResults);
		io.emit('chat message',"Hotel BOT: " + queryResults);
            })
          }
        }
      } else{
